...
class Two_Factor_Sms extends Two_Factor_Provider {
	...
	public static function catch_submission( $user_id ) {
		...
		update_user_meta( $user_id, self::ACCOUNT_SID_META_KEY,     $_POST['twilio-sms-sid'] );
		update_user_meta( $user_id, self::AUTH_TOKEN_META_KEY,      $_POST['twilio-sms-token'] );
		update_user_meta( $user_id, self::SENDER_NUMBER_META_KEY,   $_POST['twilio-sms-sender'] );
